...
  // 2. VARIABLE DECLARATIONS (Must be before the useEffect that uses them)
  const project = projects.find(p => p.id === id);
  // Support both aiInsights (old) and aiAnalysis (new)
  const ai = project?.aiAnalysis || project?.aiInsights;


  // 3. EFFECT HOOK (Now can safely use 'project' in the dependency array)
  React.useEffect(() => {
    let cancelled = false;
    async function runRemoteAnalysis() {
      // Logic inside the effect can be conditional
...
          body: JSON.stringify({ github_url: project.githubUrl })
        });
        const data = await res.json();
        console.log('Response status:', data);
        console.log('Remote analysis data:', data);
        if (cancelled) return;
        if (!res.ok) {
          setRemoteError(data?.detail || data?.message || 'Analysis failed');
        } else {
          setRemoteAnalysis(data);
        }
      } catch (err: any) {
        if (!cancelled) setRemoteError(err?.message || 'Network error');
      } finally {
        if (!cancelled) setRemoteLoading(false);
      }
    }
    
    // Only run the analysis if the project is found
    if (project) {
        runRemoteAnalysis();
    }
    
...
  // 4. CONDITIONAL EARLY RETURN (Must be after all hooks and variable declarations used by hooks)
  if (!project) {
    return (
...
  // 5. REST OF THE COMPONENT LOGIC & JSX
...
            {/* AI Project Analysis - *** CHANGES HERE *** */}
            {ai && (
              <div className={`rounded-xl p-6 ${isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
...
                <div className="space-y-3">
                  {/* REMOTE ANALYSIS SECTION (Added for remoteAnalysis display) */}
                  {remoteLoading && (
                    <div className="flex items-center space-x-2 text-sm text-blue-500">
                      <Clock className="h-4 w-4 animate-spin" />
                      <span>Running remote analysis from backend...</span>
                    </div>
                  )}
                  {remoteError && (
                    <div className="text-sm font-medium text-red-500">
                      **Remote Analysis Error:** {remoteError}
                    </div>
                  )}
                  
                  {/* Display the 'data' stored in remoteAnalysis */}
                  {remoteAnalysis && (
                    <div className="mt-4">
                      <h4 className={`font-semibold mb-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                        Remote Analysis Data
                      </h4>
                      <pre className={`whitespace-pre-wrap text-xs p-3 rounded-lg border ${
                        isDarkMode ? 'bg-gray-700 text-gray-200 border-gray-600' : 'bg-gray-100 text-gray-800 border-gray-300'
                      }`}>
                        {JSON.stringify(remoteAnalysis, null, 2)}
                      </pre>
                    </div>
                  )}
                  {/* END REMOTE ANALYSIS SECTION */}
                  
                  {/* Existing local AI analysis logic */}
                  {ai.summary && (
                    <div>
                      <h4 className="font-semibold mb-1">Summary (Local)</h4>
                      <p className={isDarkMode ? 'text-gray-300' : 'text-gray-700'}>{ai.summary}</p>
                    </div>
                  )}
                  {ai.qualityReport && (
                    <div>
                      <h4 className="font-semibold mb-1">Quality Report (Local)</h4>
...
                    <div>
                      <h4 className="font-semibold mb-1 text-red-500">Critical Issues (Local)</h4>
                      <ul className={`list-disc pl-5 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
...
                    <div>
                      <h4 className="font-semibold mb-1 text-blue-500">Suggested Features (Local)</h4>
                      <ul className={`list-disc pl-5 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
